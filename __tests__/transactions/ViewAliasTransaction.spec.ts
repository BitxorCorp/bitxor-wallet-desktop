 ...
 *
 */
import { AliasAction, Deadline, TokenAliasTransaction, TokenId, NamespaceId, NetworkType } from 'bitxor-sdk';
...
describe('transactions/ViewAliasTransaction', () => {
    describe('use() should', () => {
        test('populate token alias transaction fields', () => {
            const namespaceId = new NamespaceId('alias');
            const tokenId = new TokenId('747B276C30626442');
            const alias = TokenAliasTransaction.create(
        ...
        // XXX test recognition of Namespace vs TokenId for tokens
    });
});
