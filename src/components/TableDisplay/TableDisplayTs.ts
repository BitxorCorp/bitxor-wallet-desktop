...
import { TokenTableService } from '@/services/AssetTableService/TokenTableService';
...
@Component({
    ...
    computed: {
        ...mapGetters({
            ...
            isFetchingTokens: 'token/isFetchingTokens',
...
export class TableDisplayTs extends Vue {
    ...
    /**
     * target token or namespace metadata view
     * @type {MetadataModel[]}
     */
    ...
    public isFetchingTokens: boolean;
     ...
     *     tokenSupplyChangeTransaction: boolean
     *   }}
     */
    protected modalFormsVisibility: {
        ...
        tokenSupplyChangeTransaction: false,
    };
    ...
    protected getService(): AssetTableService {
        switch (this.assetType) {
            case TableAssetType.Token:
                return new TokenTableService(this.currentHeight, this.holdTokens, this.networkConfiguration);
    ...
    private async refresh(): Promise<void> {
        switch (this.assetType) {
            case TableAssetType.Token:
                await this.$store.dispatch('token/LOAD_TOKENS');
                break;
    ...
    protected showAliasForm(rowValues: Record<string, string>): void {
        // populate asset form modal props if asset is a token
        if (this.assetType === 'token') {
            this.modalFormsProps.namespaceId = rowValues.name !== 'N/A' ? new NamespaceId(rowValues.name) : null;
            this.modalFormsProps.aliasTarget = new TokenId(rowValues.hexId);
         ...
         * @param {('address' | 'token')} aliasType
         * @returns {(TokenId | Address)}
         */
        const getInstantiatedAlias = (aliasType: string, aliasTarget: string): TokenId | Address => {
            if (aliasType === 'token') {
                return new TokenId(aliasTarget);
            }
    ...
    /**
     * Triggers the modify token supply form modal
     * @protected
     * @param {Record<string, string>} rowValues
     * @return {void}
     */
    protected showModifyTokenSupplyForm(rowValues: Record<string, string>): void {
        this.modalFormsProps.tokenId = new TokenId(rowValues.hexId);
        Vue.set(this.modalFormsVisibility, 'tokenSupplyChangeTransaction', true);
    }
