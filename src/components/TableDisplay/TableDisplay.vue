    ...
    <div class="table-container">
            ...
            <div class="table-title-container section-title">
                <div class="user-operation">
                    ...
                    <Checkbox v-if="assetType !== 'metadata'" v-model="showExpired" class="table-filter-item-container">
                        <span v-show="assetType === 'token'" style="margin-left: 0.1rem;">{{ $t('show_expired_tokens') }}</span>
        ...
        <div
            :class="[
                ...
                assetType !== 'metadata' ? (assetType === 'token' ? 'token-columns' : 'namespace-columns') : 'metadata-columns',
            ]"
        >
            <div
        ...
        <div class="table-body-container">
            ...
            >
                <div class="table-rows-container">
                    <TableRow
                        ...
                        @on-show-token-supply-change-form="showModifyTokenSupplyForm"
            ...
            <div v-if="!isLoading && (!displayedValues || displayedValues.length === 0)" class="no-data-outer-container">
                <!--<div class="no-data-message-container">
                    <div>
                        {{ assetType === 'token' ? $t('no_data_tokens') : $t('no_data_namespaces') }}
                    </div>
                </div>-->
        ...
        <ModalFormWrap
            v-if="modalFormsVisibility.tokenSupplyChangeTransaction"
            :hide-footer="true"
            :visible="modalFormsVisibility.tokenSupplyChangeTransaction"
            title="modal_title_token_supply_change"
            @close="closeModal('tokenSupplyChangeTransaction')"
        >
            <template v-slot:form>
                <FormTokenSupplyChangeTransaction
                    :token-hex-id="modalFormsProps.tokenId.toHex()"
                    class="padding-bottom"
                    @on-confirmation-success="closeModal('tokenSupplyChangeTransaction')"
                />
            </template>
        </ModalFormWrap>
