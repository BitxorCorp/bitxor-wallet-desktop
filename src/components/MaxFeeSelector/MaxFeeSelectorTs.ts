...
import { TransactionFees } from 'bitxor-sdk';
...
export class MaxFeeSelectorTs extends Vue {
     ...
     * @var {string}
     */
    private networkTokenName: string;
    private feesConfig: {
        fast: number;
        median: number;
        slow: number;
    ...
    private getLabel([key, value]: [string, number]) {
        ...
        if (value === this.feesConfig.median) {
            return this.formatLabel('fee_speed_' + key, this.averageFee, this.networkTokenName, !!this.averageFee);
        } else if (value === this.feesConfig.fast) {
            return this.formatLabel('fee_speed_' + key, this.fastFee, this.networkTokenName, !!this.fastFee);
        } else if (value === this.feesConfig.slow) {
            return this.formatLabel('fee_speed_' + key, this.slowFee, this.networkTokenName, !!this.slowFee);
        } else if (value === this.feesConfig.slowest) {
            return this.formatLabel('fee_speed_' + key, this.slowestFee, this.networkTokenName, !!this.slowestFee);
        } else {
            return this.formatLabel('fee_speed_' + key, value, this.networkTokenName);
        }
    }
     ...
     * @param showAmount
     */
    private formatLabel(labelKey: string, fee: number, token: string, showAmount: boolean = true): string {
        let label = this.$t(labelKey).toString();
        if (showAmount) {
            label += `: ${this.getFormattedRelative(fee)} ${token}`;
        }
        return label;
    }
    ...
    /**
     * Convert a relative amount to absolute using tokenInfo
     * @param {number} price
     * @return {number}
     */
