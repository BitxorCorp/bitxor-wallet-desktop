...
export class TransactionRowTs extends Vue {
     ...
     * @private
     */
    protected networkToken: TokenId;
    ...
    public getTokensIcon(): string {
        return officialIcons.tokens;
    }
    ...
    /**
     * Returns the BXR amount to be shown.
     */
    public getAmount(): number {
        if (this.transaction.type === TransactionType.TRANSFER) {
            const transferTransaction = this.transaction as TransferTransaction;
            const amount = transferTransaction.tokens.find((token) => token.id.equals(this.networkToken)).amount.compact() || 0;
    ...
    /**
     * Returns the token id of the balance or undefined for the network.
     */
    public getAmountTokenId(): TokenId | NamespaceId | undefined {
        if (this.transaction.type === TransactionType.TRANSFER) {
            const transferTransaction = this.transaction as TransferTransaction;
            return transferTransaction.tokens.find((token) => token.id.equals(this.networkToken)).id || undefined;
        }
        return undefined;
    }
    ...
    public isAmountShowTicker(): boolean {
        // if (this.transaction.type === TransactionType.TRANSFER) {
        //   const transferTransaction = this.transaction as TransferTransaction
        //   return !!transferTransaction.tokens.length
        // }
        // return true
        return false;
    }
    ...
    /**
     * Check the transaction included network token (BXR).
     * @returns boolean
     */
    public hasNetworkToken(): boolean {
        if (this.transaction.type === TransactionType.TRANSFER) {
            const transferTransaction = this.transaction as TransferTransaction;

            return transferTransaction.tokens.filter((token) => token.id.equals(this.networkToken)).length > 0;
        }
        return false;
    }

    /**
     * Check the transaction included Non network token (BXR).
     * @returns boolean
     */
    public hasNonNativeToken(): boolean {
        if (this.transaction.type === TransactionType.TRANSFER) {
            const transferTransaction = this.transaction as TransferTransaction;
    ...
    /**
     * Gets tokens list excluded network tokens.
     * @returns
     */
    public nonNativeTokenList(): TooltipTokens[] {
        if (this.transaction.type === TransactionType.TRANSFER) {
            const transferTransaction = this.transaction as TransferTransaction;

            return transferTransaction.tokens
                .filter((token) => !token.id.equals(this.networkToken))
                .map((token) => {
                    const tokenInfo = this.balanceTokens.find((tokenInfo) => tokenInfo.tokenIdHex === token.id.toHex());

                    if (tokenInfo) {
                        return {
                            name: tokenInfo.name || tokenInfo.tokenIdHex,
                            relativeAmount: Formatters.formatNumber(token.amount.compact() / Math.pow(10, tokenInfo.divisibility)),
                        };
                    } else {
                        return {
                            name: token.id.toHex(),
                            relativeAmount: Formatters.formatNumber(token.amount.compact()),
                        };
                    }
                });
        }
