...
export default class TransactionOptinPayoutDetails extends Vue {
    ...
    private get referenceInnerTransactions(): Array<TransferTransaction> {
        ...
        const transactions = innerTransactions.filter(
            (innerTransaction) =>
                ...
                (innerTransaction as TransferTransaction).tokens?.length,
        );
    ...
    private get transferredTokens(): Array<Token> {
        return this.referenceInnerTransactions.map((transaction) => transaction.tokens[0]);
    }

    private get amount(): UInt64 {
        let sumAmount = UInt64.fromNumericString('0');
        this.transferredTokens?.forEach((token) => (sumAmount = sumAmount.add(token.amount)));

        return sumAmount;
    }

    private get tokenId(): UnresolvedTokenId {
        return this.transferredTokens[0]?.id;
    }
...
.title-text {
    font-family: @bitxorFontBold;
    font-size: 40px;
    margin: 0;
}

.content-text {
    font-family: @bitxorFontMedium;
    margin: 16px 0;
    line-height: 24px;
}

.text-description {
    font-family: @bitxorFontSemiBold;
