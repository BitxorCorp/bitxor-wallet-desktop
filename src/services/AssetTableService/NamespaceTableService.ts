...
import { AliasType } from 'bitxor-sdk';
...
export class NamespaceTableService extends AssetTableService {
     ...
     * @returns {('N/A' | 'address' | 'token')}
     */
    private getAliasType(namespaceModel: NamespaceModel): 'N/A' | 'address' | 'token' {
        if (!namespaceModel.aliasTargetAddressRawPlain && !namespaceModel.aliasTargetTokenIdHex) {
            return 'N/A';
        }
        return namespaceModel.aliasType === AliasType.Address ? 'address' : 'token';
    }
    ...
    private getAliasIdentifier(namespaceModel: NamespaceModel): string {
        return namespaceModel.aliasTargetTokenIdHex || namespaceModel.aliasTargetAddressRawPlain || 'N/A';
    }
