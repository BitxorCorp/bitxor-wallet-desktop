 ...
 *
 */
import { Address, Listener, TransactionStatusError } from 'bitxor-sdk';
...
export class RESTService {
    public static async getPrice(symbol: string, peer: string): Promise<number> {
        try{  const baseURL = 'https://api.bxrscan.com/apiv1/price';
            const res = await fetch(`${baseURL}/${symbol.toUpperCase() + '_' + peer.toUpperCase()}`);
            if (res.ok) {
    ...
    public static async getPriceBxr(idToken: string): Promise<number> {
        return 0;
    }

    public static async getPrefixToken(idToken: string): Promise<string> {
        try
        {const baseURL = 'https://api.bxrscan.com/apiv1/token';
            const res = await fetch(`${baseURL}/${idToken.toUpperCase()}`);
            if (res.ok) {
                const data = await res.json();
                if (data.length > 0) {
