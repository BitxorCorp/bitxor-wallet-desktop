 ...
 *
 */
import { Account, PublicAccount, Address, NetworkType, Password, SimpleWallet, Crypto } from 'bitxor-sdk';
...
export class AccountService {
    ...
    public getAccountByPath(mnemonic: MnemonicPassPhrase, networkType: NetworkType, path: string): Account {
        ...
        const extendedKey = ExtendedKey.createFromSeed(mnemonic.toSeed().toString('hex'), Network.BITXOR);
     ...
     * @return {ExtendedKey}
     */
    public getExtendedKeyFromMnemonic(mnemonic: MnemonicPassPhrase, curve = Network.BITXOR): ExtendedKey {
     ...
     */
    public async getLedgerAccounts(networkType: NetworkType, count: number = 10, curve = Network.BITXOR): Promise<Address[]> {
     ...
     */
    public async getLedgerPublicKey(networkType: NetworkType, count: number = 10, curve = Network.BITXOR): Promise<string[]> {
        const isOptinLedgerWallet = curve === Network.BITCOIN;
        const derivationService = new DerivationService(networkType);
