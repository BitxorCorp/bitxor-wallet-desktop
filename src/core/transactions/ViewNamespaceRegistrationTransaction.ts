            ...
            return [
                { key: 'namespace_name', value: rootNamespaceName },
                {
                    ...
                    value:
                        duration.compact() < 1
                            ? 'INFINITY'
                            : TimeHelpers.durationToRelativeTime(parseInt(duration.toString()), blockGenerationTargetTime),
                },
                {
                    key: 'estimated_rental_fee',
                    value: {
                        amount:
                            this.transaction['duration'].compact() < 1
                                ? this.$store.getters['network/rentalFeeEstimation'].effectiveRootNamespaceEternalFee.compact()
