...
// configuration
import { Transaction, SignedTransaction, Convert, CosignatureSignedTransaction, AggregateTransaction } from 'bitxor-sdk';
 ...
 * const sym = new BitxorLedger();
 "44'/4343'/account'/change/accountIndex"
 */

export class BitxorLedger {
    transport: any;
    ...
    /**
     * Return true if app version is above the supported Bitxor app version
     * @return {boolean}
     */
    ...
    /**
     * Get Bitxor app version on Ledger device

     * @return an object contain major, minor, patch version of the Bitxor app on Ledger device
     */
    ...
    /**
     * get Bitxor's address for a given BIP 44 path from the Ledger
     *
     * @param path a path in BIP 44 format
     * @param display optionally enable or not the display
     * @param chainCode optionally enable or not the chainCode request
     * @param isOptinBitxorWallet if Opt-in Bitxor wallet uses curve Secp256K1 else uses curve Ed25519
    ...
    /**
     * sign a Bitxor transaction by account on Ledger at given BIP 44 path
     *
     * @param path a path in BIP 44 format
    ...
    /**
     * sign a Bitxor Cosignature transaction with a given BIP 44 path
     *
     * @param path a path in BIP 44 format
    ...
    public async signCosignatureTransaction(
        ...
        const rawTx = Buffer.from(signingBytes, 'hex');
        const response = await this.ledgerMessageHandler(path, rawTx, false, isOptinBitxorWallet);
        // Response from Ledger
        const h = response.toString('hex');
        const signature = h.slice(0, 128);
     ...
     */
    private async ledgerMessageHandler(path: string, rawTx: Buffer, chainCode: boolean, isOptinBitxorWallet: boolean) {
        const TX_INS_FIELD = 0x04;
        const MAX_CHUNK_SIZE = 255;
        const CONTINUE_SENDING = '0x9000';

        const curveMask = isOptinBitxorWallet ? 0x40 : 0x80;
        const bipPath = BIPPath.fromString(path).toPathArray();
        const apduArray = [];
        let offset = 0;
