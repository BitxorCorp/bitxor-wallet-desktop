...
import { AggregateTransaction, Transaction, TransactionType } from 'bitxor-sdk';
...
export class CSVHelpers {
    ...
    private static constructTransactionsObject(transaction: Transaction) {
        ...
        for (let i = 0; i < value.length; i++) {
            ...
            } else if (value[i].key.startsWith('Token')) {
                result[value[i].key] = value[i].value.amount + ' (' + value[i].value.tokenHex + ')';
            } else {
