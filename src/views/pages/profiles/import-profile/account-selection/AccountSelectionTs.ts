...
import { AccountInfo, Address, TokenId, NetworkType, Password, RepositoryFactory } from 'bitxor-sdk';
import { MnemonicPassPhrase, Network } from 'bitxor-hd-wallets';
...
export default class AccountSelectionTs extends Vue {
        ...
        try {
            ...
            if (repositoryFactory) {
                ...
                this.addressBalanceMap = {
                    ...this.addressBalanceMap,
                    ...this.mapBalanceByAddress(accountsInfo, this.networkToken, this.addressesList),
                };
            } else {
                this.$store.dispatch('notification/ADD_ERROR', 'bitxor_node_cannot_connect');
                this.addressBalanceMap = { ...this.mapBalanceByAddress(undefined, this.networkToken, this.addressesList) };
                return;
            }
        } catch (error) {
            console.log(error);
        ...
        try {
            ...
            if (repositoryFactory) {
                ...
                this.optInAddressBalanceMap = {
                    ...this.optInAddressBalanceMap,
                    ...this.mapBalanceByAddress(accountsInfo, this.networkToken, optInAddresses),
                };
            } else {
                this.optInAddressBalanceMap = {
                    ...this.mapBalanceByAddress(undefined, this.networkToken, optInAddresses),
                };
                this.$store.dispatch('notification/ADD_ERROR', 'bitxor_node_cannot_connect');
                return;
            }
    ...
    public mapBalanceByAddress(accountsInfo: AccountInfo[], tokenId: TokenId, addressList: Address[]): { [address: string]: number } {
        const addressToAccountInfo = _.keyBy(accountsInfo, (a) => a.address.plain());
        return _.chain(addressList)
            .keyBy((a) => a.plain())
            .mapValues((a) => addressToAccountInfo[a.plain()]?.tokens.find((m) => m.id.equals(tokenId))?.amount.compact() ?? 0)
            .value();
    }
