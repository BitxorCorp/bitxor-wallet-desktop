 ...
 *
 */
import { Address, MetadataType, PublicAccount, RepositoryFactory, Transaction } from 'bitxor-sdk';
// @ts-ignore
...
export class FormMetadataCreationTs extends FormTransactionBase {
    ...
    /**
     * Value for token and namespace selectboxes
     * @type {MetadataModel}
     */
    @Prop({
        default: null,
     ...
     * @type {TokenModel[]}
     */
    protected ownedTokens: TokenModel[];
    ...
    protected isToken(): boolean {
        return this.type === MetadataType.Token;
    }
    ...
    protected resetForm() {
        ...
        // for tokens and namespaces, target account will be the signer account's itself (as hidden)
    ...
    get targetLabel(): string {
        ...
        switch (this.type) {
            case MetadataType.Token:
                title = 'form_label_target_token_id';
                break;
    ...
    get targetIdValidatorName(): string {
        return this.isToken() ? 'token_id' : 'namespace_id';
    }

    private get availableTokens(): TokenModel[] {
        return this.ownedTokens.filter((entry) => {
            const expiration = TokenService.getExpiration(entry, this.currentHeight, this.networkConfiguration.blockGenerationTargetTime);
    ...
    /**
     * Get targeted token or namespace name
     */
    public targetNameById(targetId: string): string {
        ...
        if (this.isToken()) {
            const targetToken = this.ownedTokens.find((token) => token.tokenIdHex === targetId);
    ...
    private updateFormItems(selectedItem: MetadataModel) {
        if (selectedItem) {
            ...
            this.formItems.targetId = this.type === MetadataType.Token ? selectedItem.targetId : this.formItems.targetName;
