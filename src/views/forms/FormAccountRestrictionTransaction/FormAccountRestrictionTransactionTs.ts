...
@Component({
    ...
    computed: {
        ...mapGetters({
            ...
            accountTokenRestrictions: 'restriction/accountTokenRestrictions',
...
export class FormAccountRestrictionTransactionTs extends FormTransactionBase {
    ...
    public accountTokenRestrictions: AccountRestriction[];
    ...
    protected resetForm() {
        ...
        if (this.restrictionToBeDeleted) {
            ...
            } else if (this.restrictionTxType === AccountRestrictionTxType.TOKEN) {
                this.formItems.tokenIdRaw = this.restrictionToBeDeleted.value;
            } else {
    ...
    private createTransaction(restrictionTxType: AccountRestrictionTxType): Transaction {
        switch (this.restrictionTxType) {
            ...
            case AccountRestrictionTxType.TOKEN: {
                const toBeAdded = this.isDeleteMode ? [] : [new TokenId(this.formItems.tokenIdRaw)];
                const toBeDeleted = this.isDeleteMode ? [new TokenId(this.formItems.tokenIdRaw)] : [];

                return AccountRestrictionTransaction.createTokenRestrictionModificationTransaction(
                    this.createDeadline(),
                    RestrictionFlagMapping.toRestrictionFlag(
                        restrictionTxType,
                        this.formItems.direction,
                        this.formItems.blockType,
                    ) as TokenRestrictionFlag,
                    toBeAdded,
                    toBeDeleted,
                    this.networkType,
                    UInt64.fromUint(this.formItems.maxFee),
    ...
    protected get transactionTypeDisabled(): boolean {
        ...
        if (this.restrictionTxType === AccountRestrictionTxType.ADDRESS) {
            ...
            restrictions = this.accountTokenRestrictions;
        } else {
