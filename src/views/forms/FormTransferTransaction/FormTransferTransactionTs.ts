...
import { TokenInputsManager } from './TokenInputsManager';
...
// @ts-ignore
import TokenAttachmentInput from '@/components/TokenAttachmentInput/TokenAttachmentInput.vue';
// @ts-ignore
...
export class FormTransferTransactionTs extends FormTransactionBase {
    ...
    public formItems = {
        signerAddress: '',
        attachedTokens: new Array<TokenAttachment>(),
        recipientRaw: '',
        recipient: null,
        selectedTokenHex: '',
        relativeAmount: 0,
    ...
    protected tokenInputsManager: TokenInputsManager;

    private balanceTokens: TokenModel[];
    ...
    protected resetForm(): void {
        ...
        this.formItems.selectedTokenHex = this.networkToken.toHex();
        ...
        this.resetTokensAndTriggerChange(this.importTransaction, true);
    }

    /**
     * Resetting tokens list when signer is changed
     ...
     * @return {void}
     */
    private resetTokensAndTriggerChange(importedTransaction: boolean, triggerChange = false): void {
        this.tokenInputsManager = TokenInputsManager.initialize(this.currentTokenList());
        // - reset attached tokens
        this.formItems.attachedTokens = [];
        this.formItems.selectedTokenHex = this.networkToken.toHex();

        const attachedTokens: TokenAttachment[] = [
            {
                id: new TokenId(this.networkCurrency.tokenIdHex),
                tokenHex: this.networkCurrency.tokenIdHex,
        ...
        if (importedTransaction) {
            ...
            this.formItems.attachedTokens.forEach((attachedToken) => {
                this.tokenInputsManager.setSlot(attachedToken.tokenHex, attachedToken.uid);
            });
            this.onChangeRecipient();
        } else {
            attachedTokens.forEach((attachedToken, index) => {
                this.tokenInputsManager.setSlot(attachedToken.tokenHex, attachedToken.uid);
                Vue.set(this.formItems.attachedTokens, index, attachedToken);
            });
        }
        if (triggerChange) {
            this.triggerChange();
    ...
    /**
     * Returns the token list of the current account or current signer
     * depending on the multisig situation
     * @protected
     * @returns
     */
    protected currentTokenList(): TokenModel[] {
        // filter out expired tokens
        return this.balanceTokens.filter((tokenInfo) => {
    ...
    protected getTransactions(): TransferTransaction[] {
        const tokensInfo = this.$store.getters['token/tokens'] as TokenModel[];

        // Push network currency info for offline transaction format amount to absolute
        if (tokensInfo.length === 0) {
            tokensInfo.push({
                tokenIdHex: this.networkCurrency.tokenIdHex,
        ...
        const tokens = this.formItems.attachedTokens
            ...
            .map(
                (attachment): Token => {
                    ...
                    const info = tokensInfo.find((i) => i.tokenIdHex === attachment.tokenHex);
                    const div = info ? info.divisibility : 0;
                    // - format amount to absolute
                    return new Token(new TokenId(RawUInt64.fromHex(attachment.tokenHex)), UInt64.fromUint(amount * Math.pow(10, div)));
                },
            );
        return [
    ...
    protected setTransactions(transactions: TransferTransaction[]) {
        ...
        // - populate attached tokens
        this.formItems.attachedTokens = this.tokensToAttachments(transaction.tokens);
    ...
    /**
     * Hook called when the child component TokenAttachmentDisplay triggers
     * the event 'delete'
     * @return {void}
     */
    public onDeleteToken(id: TokenId) {
        const updatedAttachedTokens = [...this.formItems.attachedTokens].filter(({ tokenHex }) => tokenHex !== id.toHex());

        // fixes reactivity on attachedTokens (observer resolution)
        Vue.set(this.formItems, 'attachedTokens', updatedAttachedTokens);
    }
     ...
     * @return {void}
     */
    protected onTokenInputChange(payload: { tokenAttachment: TokenAttachment; inputIndex: number }): void {
        const { tokenAttachment, inputIndex } = payload;

        // set slot
        this.tokenInputsManager.setSlot(tokenAttachment.tokenHex, inputIndex);

        // update formItems
        const newAttachedTokens = [...this.formItems.attachedTokens];
        const indexToUpdate = newAttachedTokens.findIndex(({ uid }) => uid == inputIndex);
        newAttachedTokens[indexToUpdate] = tokenAttachment;
        Vue.set(this.formItems, 'attachedTokens', newAttachedTokens);
        this.triggerChange();
    }

    /**
     * Handle deletion of a token input
     * @protected
     * @param {number} inputIndex
     */
    protected onDeleteTokenInput(index: number): void {
        // unset token input slot
        this.tokenInputsManager.unsetSlot(index);

        // update formItems, set input uid to null
        const indexToUpdate = this.formItems.attachedTokens.findIndex(({ uid }) => uid == index);
        Vue.set(this.formItems.attachedTokens, indexToUpdate, { uid: null });
        // delete the last one in order to re-render the list
        this.formItems.attachedTokens.pop();
        this.triggerChange();
    }

    /**
     * Internal helper to format a {Token} entry into
     * an array of TokenAttachment used in this form.
     * @internal
     * @param {Token[]} tokens
     * @return {TokenAttachment[]}
     */
    private tokensToAttachments(tokens: Token[]): TokenAttachment[] {
        return tokens
            .map((token) => {
                const info = this.balanceTokens.find((m) => token.id.toHex() === m.tokenIdHex);
                if (!info) {
                    return null;
                }
                // amount will be converted to RELATIVE
                return {
                    id: new TokenId(info.tokenIdHex), // XXX resolve tokenId from namespaceId
                    tokenHex: info.tokenIdHex, // XXX resolve tokenId from namespaceId
                    name: info.name,
                    amount: (Number(token.amount.toString()) / Math.pow(10, info.divisibility)).toLocaleString(undefined, {
    ...
    /**
     *  Hook called when adding a new token attachment input
     * @protected
     */
    protected addTokenAttachmentInput(): void {
        if (!this.tokenInputsManager.hasFreeSlots()) {
            return;
        }

        // generate id and prepare token for slot
        const uid = Math.floor(Math.random() * 10e6);
        const [tokenToAffectToNewInput] = this.tokenInputsManager.getTokensBySlot(uid);

        // update inputs manager and form items
        this.tokenInputsManager.setSlot(tokenToAffectToNewInput, uid);
        this.formItems.attachedTokens.push({
            tokenHex: tokenToAffectToNewInput,
            amount: '0',
            uid,
        });
        ...
        if (AddressValidator.validate(this.formItems.recipientRaw)) {
            ...
            if (this.transactions) {
                ...
                this.transactions.map((item: TransferTransaction) => {
                    data.push({
                        transaction: item,
                        attachments: this.tokensToAttachments(item.tokens),
                    });
                });
    ...
    public async signerChanged(address: string) {
        ...
        this.resetTokensAndTriggerChange(false, true);
    }
    ...
    public emitToAggregate() {
        if (this.transactions && this.transactions[0]) {
            this.formItems.tokens = this.transactions[0].tokens;
    ...
    async created() {
        ...
        if (this.isAggregate && this.value) {
            ...
            if (this.formItems.attachedTokens.length) {
                this.tokenInputsManager = TokenInputsManager.initialize(this.currentTokenList());
                this.formItems.attachedTokens.forEach((m) => this.tokenInputsManager.setSlot(m.tokenHex, m.uid));
            }
        }
