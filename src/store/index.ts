    ...
    actions: {
        async initialize({ dispatch, getters }) {
            const callback = async () => {
                ...
                // Network init must happen before Token init because network currency Ids
                // are supplied to TokenService from the network configuration
                await dispatch('network/initialize');
                await dispatch('metadata/initialize');
                await dispatch('token/initialize');
