 ...
 *
 */
import { AccountRestriction, Address, TokenId, RepositoryFactory } from 'bitxor-sdk';
import Vue from 'vue';
...
export default {
    ...
    getters: {
        ...
        accountTokenRestrictions: (state) => state.accountRestrictions.filter((r) => r.values && r.values[0] instanceof TokenId),
        accountOperationRestrictions: (state) =>
            state.accountRestrictions.filter((r) => r.values && !(r.values[0] instanceof Address) && !(r.values[0] instanceof TokenId)),
