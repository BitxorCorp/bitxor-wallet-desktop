...
export default {
    ...
    actions: {
        ...
        async ON_NEW_TRANSACTION({ dispatch }, transaction: Transaction) {
            ...
            // reload metadata list first so that tokens and namespaces can be updated
            await dispatch('metadata/LOAD_METADATA_LIST', {}, { root: true });

            // add actions to the dispatcher according to the transaction types
            if (
                [TransactionType.NAMESPACE_REGISTRATION, TransactionType.TOKEN_ALIAS, TransactionType.ADDRESS_ALIAS].some((a) =>
